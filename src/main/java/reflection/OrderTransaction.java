...
public class OrderTransaction extends MyTransaction implements IOrderHandler, LiveTransaction {
	...
	private double m_sourceTokenQty; // if set, it means that we added our source toekn quantity to the UserTokenMgr, and we must subtract it out when the order is completed
	...
	private void requireSufficientCrypto(boolean before) throws Exception {
		...
		try {
			String url = String.format( "%s/hook/get-wallet-map/%s",
					m_config.hookBaseUrl2(), m_walletAddr );
		...
		if (before) {
			...
			out( "Checking balance prior to order:  balance=%s  offset=%s  get=%s  needed=%s", 
