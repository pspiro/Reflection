...
public class TradingHours {
	...
	public void startQuery() {
		if (!m_started) {
			...
			Util.executeEvery( "TradingHours", 0, interval, () -> queryNow() );
		}
		else {
			queryNow();
		}
	...
	/** Return true if now is inside trading hours OR liquid hours. */
	boolean insideAnyHours( boolean is24, Date now) throws Exception {  // change to return boolean
		return is24 
				? insideHours( "SMART", now) || insideHours( "OVERNIGHT", now)
				: insideHours( "SMART", now);
	}
